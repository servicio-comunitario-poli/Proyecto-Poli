...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // IMPORTANTE: Se envía la materia al contexto para que se guarde en la sesión del profesor.
      const resultado = await login(credencial, password, tipoUsuario, materia);
        ...
        <div className="flex gap-2 mb-6 bg-slate-100 p-1 rounded-lg">
          <button 
            onClick={() => {
              ...
              setMateria(''); // Reiniciamos la materia si vuelve a estudiante
            }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {tipoUsuario === 'admin' && (
            <div className="animate-fadeIn">
              <label className="block text-sm font-bold text-slate-700 mb-1">Materia (Solo Profesores)</label>
              <select
                value={materia}
                onChange={(e) => setMateria(e.target.value)}
                className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-rose-900 focus:border-rose-900 outline-none transition-colors bg-white text-slate-700"
              >
                <option value="">Administrador General (Sin materia)</option>
